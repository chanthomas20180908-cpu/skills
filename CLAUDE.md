...
└── references/                # Templates, schemas, and reference data
    ...
    ├── reference-example.yaml # Human-readable filled example (do not copy prose)
...
3. **Check `references/template.yaml`** for the agent generation skeleton and `references/reference-example.yaml` for a human-readable filled example.
...
3. Add `references/` with `schema.yaml`, `template.yaml`, `reference-example.yaml`, and any templates.
